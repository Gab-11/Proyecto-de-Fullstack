package cl.duoc.gamezone.backend.config;

import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.security.config.Customizer;
import org.springframework.security.config.annotation.web.builders.HttpSecurity;
import org.springframework.security.web.SecurityFilterChain;

@Configuration
public class SecurityConfig {

    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {

        http
                .csrf(csrf -> csrf.disable())  // Desactiva protección CSRF (para APIs)
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/api/**").permitAll()  // Nuestra API SIN autenticación
                        .anyRequest().permitAll()               // TODO lo demás libre
                )
                .httpBasic(Customizer.withDefaults())           // Evita login por defecto
                .formLogin(form -> form.disable());             // Desactiva formulario de login

        return http.build();
    }
}
